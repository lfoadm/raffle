...
public function showPaymentPage($orderId)
{
    ...
    $payload = [
        ...
        'payer' => $payer,
        'back_urls' => $backUrls,
        'notification_url' => route('webhook.mercadopago'), // constrói a url de retorno do mercado pago
    ...
    $response = Http::withToken(config('services.mercadopago.access_token'))->post('https://api.mercadopago.com/checkout/preferences', $payload);
    ...
    //dd("chegou aqui", $items, $payer, $backUrls, $payload, $response);
    ...
    public function webhook(Request $request)
    {
        ...
        if ($response->status == 'approved') {
            ...
            // Conclui as cotas como pagas
            foreach ($order->items as $item) {
                RaffleQuota::whereIn('id', $item->raffle_quotas->pluck('id'))->update(['status' => 'sold']);
            }
